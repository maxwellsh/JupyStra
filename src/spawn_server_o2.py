...
def prompt_password():
    password = getpass.getpass("O2 password: ")
    return password

class Spawner(object):
    """ A class for spanwing Jupyter notebook servers on O2
    """
    ...
    def _connect(self):
        ...
        try:
            ssh.connect('o2.hms.harvard.edu', username=self.login, password=self.password)

        # Prompt for a password on failure
        except paramiko.ssh_exception.SSHException:
            self.password = getpass.getpass("O2 password: ")
            ssh.connect('o2.hms.harvard.edu', username=self.login, password=self.password)
    ...
    def submit_server_job(self):
        print("Establishing Jupyter server on O2")
    ...
    def get_exec_host(self):
        ...
        stdin, stdout, stderr = self.exec_cmd('bash -l -c "squeue -j {}"'.format(self.jobID), verbose=False, output=True)

        # only interested in the second line
        stdout.readline()
        out = stdout.readline()
        self.exec_host = out.split()[7]

        print("Server is now running on {}:{}".format(self.exec_host, self.args.port))

    def write_shell_script(self):
        sw = script_writer.ShellWriter(self.exec_host, port_local=self.args.local_port, port_remote=self.args.remote_port, port_jup=self.args.port, server="o2.hms.harvard.edu")
    ...
    def connect_to_jup_server(self):
        sw = script_writer.ShellWriter(self.exec_host, port_local=self.args.local_port, port_remote=self.args.remote_port, port_jup=self.args.port, username=self.login, server='o2.hms.harvard.edu')
        cmd = sw.get_ssh_cmd()
